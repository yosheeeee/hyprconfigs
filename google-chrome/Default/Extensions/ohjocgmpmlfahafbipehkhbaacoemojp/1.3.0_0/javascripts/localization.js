...
Localization.setLocale = function(locale) {
    Utils.setStorage({"_locale": locale})
}


Localization.resetI18n = async function(mainCallback) {
    ...
    chrome.i18n = (function() {
        ...
        var theI18nObject = {
            ...
            _getL10nData: function(mainCallback) {
                ...
                (async () => {
                    const thelocale = await Utils.getStorage('_locale');
                    if (!thelocale) {
                        Localization.setLocale(navigator.language.substring(0, 2))
                    }
                    result.locales.push(thelocale);
